...
int is_leaf(xmlNode * node)
{
 ...
 dk *extract_bmd (char * filename,  char* fields[]){
 ...
 //extracts payload node
  ...
  abc->MessageID=attributes[0].value;
  abc->MessageType=attributes[1].value;
  abc->Sender=attributes[2].value;
  abc->Destination=attributes[3].value;
  abc->CreationDateTime=attributes[4].value;
  abc->Signature=attributes[5].value;
  abc->ReferenceID=attributes[6].value;
  abc->Payload=attributes[7].value;
  abc->UserProperties=attributes[8].value;
...
/*int main(){
  ...
  char * fields[10];
  struct dk xyz;
  xyz=extract_bmd(filename, fields); 
